      ...
      {top_three.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400 font-bold">Gold, Silver &amp; Bronze Podium</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 items-end">
            ...
            {firstPlace && (
              <div className="bg-gradient-to-b from-amber-950/80 via-slate-950 to-slate-950 text-white rounded-3xl p-6 sm:p-7 border-2 border-amber-400 shadow-2xl shadow-amber-500/25 flex flex-col justify-between order-1 md:order-2 md:-translate-y-4 relative overflow-hidden group hover:scale-[1.02] transition-all">
                <div className="absolute top-3 right-3 text-2xl font-black text-amber-400 opacity-60">#1</div>
                <div className="space-y-4">
                  <Link href={`/profile/${encodeURIComponent(firstPlace.username)}`} className="flex items-center gap-3 group/item">
                    <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-amber-400 via-yellow-400 to-amber-600 text-slate-950 font-black text-2xl flex items-center justify-center shadow-lg border-2 border-white/60 group-hover/item:scale-105 transition-transform">
                      {firstPlace.full_name.slice(0, 1).toUpperCase()}
                    </div>
                    <div>
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-amber-400/20 text-amber-300 border border-amber-400/40 text-[10px] font-black uppercase shadow-xs">
                      ...
                      <span className="text-xs text-slate-400 font-mono">@{firstPlace.username}</span>
                    </div>
                  </Link>
                ...
                <div className="mt-6 pt-4 border-t border-slate-800 flex items-center justify-between">
                  <div className="grid grid-cols-3 gap-2 text-center flex-1">
                    ...
                    <div>
                      <span className="text-[10px] text-slate-400 block uppercase">Avg Score</span>
                      <strong className="text-base font-black text-white">{firstPlace.avg_score}</strong>
                    </div>
                  </div>
                  <Link
                    href={`/profile/${encodeURIComponent(firstPlace.username)}`}
                    className="ml-3 px-3 py-1.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 text-[11px] font-black rounded-xl transition-all flex items-center gap-1 shadow-md shadow-amber-500/20"
                  >
                    <span>Profile</span>
                    <ArrowRight className="w-3 h-3" />
                  </Link>
            ...
            {/* 3rd Place (Bronze) */}
            {thirdPlace ? (
              <div className="bg-gradient-to-b from-slate-900 to-slate-950 text-white rounded-3xl p-6 border-2 border-amber-700/50 shadow-xl shadow-slate-900/10 flex flex-col justify-between order-3 md:order-3 relative overflow-hidden group hover:border-amber-600 transition-all">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          ...
                          >
                            <div className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-black text-xs flex items-center justify-center flex-shrink-0 border border-slate-200 dark:border-slate-700 group-hover/cand:border-japan-red group-hover/cand:bg-red-50 dark:group-hover/cand:bg-slate-700 transition-colors">
                              {c.full_name.slice(0, 1).toUpperCase()}
                            </div>
                            <div>
                              <div className="flex items-center gap-1.5">
                                <span className="font-bold text-slate-900 dark:text-white group-hover/cand:text-japan-red dark:group-hover/cand:text-rose-400 transition-colors">
                                  {c.full_name}
                                </span>
                                {isCurrentUser && (
                                  <span className="px-1.5 py-0.2 bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 rounded text-[9px] font-extrabold">
                                    You
                                  </span>
                                )}
                              </div>
                              <span className="text-[11px] text-slate-400 dark:text-slate-500 block font-mono">@{c.username}</span>
                            </div>
                          </Link>
                        </td>
