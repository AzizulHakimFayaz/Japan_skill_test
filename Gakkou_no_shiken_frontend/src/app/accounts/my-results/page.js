  ...
  if (authLoading || loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-sm font-bold text-slate-600 dark:text-slate-400">Loading Candidate Statistics...</p>
      </div>
    );
  }
      ...
      {total_attempts > 0 ? (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
            <div className="bg-white dark:bg-slate-900/90 p-4 sm:p-7 rounded-2xl sm:rounded-[28px] border border-slate-200/80 dark:border-slate-800 shadow-2xs flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-5 btn-touch-active mobile-app-card">
              <div className="w-10 h-10 sm:w-13 sm:h-13 rounded-full bg-slate-100/90 dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700 flex items-center justify-center text-slate-700 dark:text-slate-300 flex-shrink-0">
                <FileText className="w-5 h-5 sm:w-6 sm:h-6 text-slate-700 dark:text-slate-300" />
              </div>
              <div className="space-y-0.5">
                <span className="text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 font-extrabold uppercase tracking-wider block">Exams</span>
                <strong className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white leading-none block">{total_attempts}</strong>
                <span className="text-[10px] sm:text-[11px] text-slate-400 font-medium block">Completed CBT</span>
              </div>
            </div>

            <div className="bg-white dark:bg-slate-900/90 p-4 sm:p-7 rounded-2xl sm:rounded-[28px] border border-slate-200/80 dark:border-slate-800 shadow-2xs flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-5 btn-touch-active mobile-app-card">
              <div className="w-10 h-10 sm:w-13 sm:h-13 rounded-full bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-100 dark:border-emerald-800 flex items-center justify-center text-emerald-600 dark:text-emerald-400 flex-shrink-0">
                <CheckCircle2 className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-600 dark:text-emerald-400" />
              </div>
              <div className="space-y-0.5">
                <span className="text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 font-extrabold uppercase tracking-wider block">Pass Rate</span>
                <strong className="text-2xl sm:text-3xl font-black text-emerald-600 dark:text-emerald-400 leading-none block">{pass_rate}%</strong>
                <span className="text-[10px] sm:text-[11px] text-slate-400 font-medium block">{passed_attempts} of {total_attempts} passed</span>
              </div>
            </div>

            <div className="bg-white dark:bg-slate-900/90 p-4 sm:p-7 rounded-2xl sm:rounded-[28px] border border-slate-200/80 dark:border-slate-800 shadow-2xs flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-5 btn-touch-active mobile-app-card">
              <div className="w-10 h-10 sm:w-13 sm:h-13 rounded-full bg-amber-50 dark:bg-amber-950/60 border border-amber-100 dark:border-amber-800 flex items-center justify-center text-amber-600 dark:text-amber-400 flex-shrink-0">
                <Target className="w-5 h-5 sm:w-6 sm:h-6 text-amber-600 dark:text-amber-400" />
              </div>
              <div className="space-y-0.5">
                <span className="text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 font-extrabold uppercase tracking-wider block">Max Score</span>
                <strong className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white leading-none block">
            ...
            <div className="bg-white dark:bg-slate-900/90 p-4 sm:p-7 rounded-2xl sm:rounded-[28px] border border-slate-200/80 dark:border-slate-800 shadow-2xs flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-5 btn-touch-active mobile-app-card">
              <div className="w-10 h-10 sm:w-13 sm:h-13 rounded-full bg-rose-50 dark:bg-rose-950/60 border border-rose-100 dark:border-rose-800 flex items-center justify-center text-japan-red dark:text-rose-400 flex-shrink-0">
                <GraduationCap className="w-5 h-5 sm:w-6 sm:h-6 text-japan-red dark:text-rose-400" />
              </div>
              <div className="space-y-0.5">
                <span className="text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 font-extrabold uppercase tracking-wider block">CEFR Level</span>
                <strong className="text-xl sm:text-2xl font-black text-japan-red dark:text-rose-400 leading-none block">{highest_level}</strong>
          ...
          <div className="bg-white dark:bg-slate-900/90 rounded-2xl sm:rounded-[28px] border border-slate-200/80 dark:border-slate-800 p-4 sm:p-8 shadow-2xs space-y-4 sm:space-y-6 mobile-app-card">
            <div>
              <span className="text-[11px] sm:text-xs font-extrabold text-japan-red dark:text-rose-400 uppercase tracking-wider block mb-0.5">
                Diagnostic Analysis
              </span>
              <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">Cumulative Section Performance</h2>
              <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">Accuracy breakdown across all 4 JFT exam sections.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6">
              {section_stats.map((sec) => (
                <div key={sec.key} className="p-4 sm:p-5 rounded-xl sm:rounded-2xl border border-slate-200/70 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/60 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <div>
                      <strong className="text-xs sm:text-sm font-extrabold text-slate-900 dark:text-white block">{sec.name_en}</strong>
                      <span className="text-[11px] text-slate-400 dark:text-slate-500 font-medium">{sec.name_ja}</span>
                    </div>
                    <span className="text-base sm:text-lg font-black text-slate-900 dark:text-white">{sec.pct}%</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 font-medium">
                    <span>
                      {sec.correct} of {sec.total} correct
                    </span>
                    <span className="font-bold text-slate-700 dark:text-slate-300">Pass: 80%</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white dark:bg-slate-900/90 rounded-2xl sm:rounded-[28px] border border-slate-200/80 dark:border-slate-800 shadow-2xs overflow-hidden mobile-app-card">
            <div className="p-4 sm:p-8 border-b border-slate-100 dark:border-slate-800">
              <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">Exam Attempt History</h2>
              <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">Record of completed practice tests.</p>
            </div>

            {/* Desktop Table View */}
            <div className="hidden sm:block overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs font-extrabold uppercase tracking-wider border-b border-slate-200/80 dark:border-slate-700">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800 font-medium text-slate-700 dark:text-slate-300">
                  {attempts.map((attempt) => (
                    <tr key={attempt.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/60 transition-colors">
                      <td className="px-6 py-4 font-bold text-slate-900 dark:text-white">{attempt.test_title}</td>
                      ...
                      <td className="px-6 py-4 font-bold font-mono text-slate-900 dark:text-white">
                        {attempt.scaled_score} <span className="text-xs text-slate-400 font-normal">/ 250</span>
                      </td>
                      <td className="px-6 py-4 font-bold text-japan-red dark:text-rose-400 text-xs">{attempt.assessment_level}</td>
                      <td className="px-6 py-4">
                        {attempt.passed ? (
                          <span className="inline-flex items-center gap-1 py-0.5 px-2.5 rounded-full text-xs font-extrabold bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                            <Check className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />
                            <span>PASS</span>
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 py-0.5 px-2.5 rounded-full text-xs font-extrabold bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800">
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link
                          ...
                          className="text-xs font-bold text-japan-red dark:text-rose-400 hover:underline py-1 px-2 rounded hover:bg-red-50 dark:hover:bg-slate-800 inline-flex items-center gap-1"
                        >
              ...
              {attempts.map((attempt) => (
                <div key={attempt.id} className="p-4 space-y-3 btn-touch-active">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <strong className="text-sm font-extrabold text-slate-900 dark:text-white block leading-snug">{attempt.test_title}</strong>
                    ...
                    {attempt.passed ? (
                      <span className="inline-flex items-center gap-1 py-0.5 px-2.5 rounded-full text-[10px] font-extrabold bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 flex-shrink-0">
                        <Check className="w-2.5 h-2.5 text-emerald-600 dark:text-emerald-400" />
                        <span>PASS</span>
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 py-0.5 px-2.5 rounded-full text-[10px] font-extrabold bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800 flex-shrink-0">
                  ...
                  <div className="flex items-center justify-between text-xs pt-1 border-t border-slate-50 dark:border-slate-800">
                    <div>
                      ...
                      <strong className="text-slate-900 dark:text-white font-mono">{attempt.scaled_score} / 250</strong>
                    </div>
                    <Link href={`/attempt/${attempt.id}`} className="text-xs font-extrabold text-japan-red dark:text-rose-400 flex items-center gap-1">
                      <span>View Scorecard</span>
                      <ArrowRight className="w-3 h-3" />
                    </Link>
                  </div>
        ...
        /* Empty State */
        <div className="bg-white dark:bg-slate-900/90 rounded-3xl border border-slate-200/90 dark:border-slate-800 p-8 sm:p-14 text-center space-y-4 shadow-sm">
          <div className="w-16 h-16 rounded-full bg-red-50 dark:bg-rose-950/60 text-japan-red dark:text-rose-400 flex items-center justify-center mx-auto">
            <FileText className="w-8 h-8 text-japan-red dark:text-rose-400" />
          </div>
          <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">No Exam Attempts Recorded Yet</h3>
