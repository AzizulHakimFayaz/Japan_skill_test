...
def landing_page_view(request):
    qs = Test.objects.filter(is_published=True).order_by("category", "created_at")
    tests_by_category = OrderedDict([
        (Test.Category.BASIC, list(qs.filter(category=Test.Category.BASIC))),
        (Test.Category.SKILL, list(qs.filter(category=Test.Category.SKILL))),
    ])
    # (category_value, display_label, chip_classes, icon_bg_class)
    section_specs = [
        (Test.Category.BASIC, "Basic Tests",
         "bg-indigo-50 text-indigo-700 border border-indigo-100",
         "bg-indigo-600"),
        (Test.Category.SKILL, "Skill Tests",
...
def quiz_page_view(request, pk):
    ...
    # Fetch questions and their answer options efficiently
...
def attempt_results_view(request, pk):
    ...
    # Prefetch questions and options for display in correct order
    questions = test.questions.prefetch_related('options')
