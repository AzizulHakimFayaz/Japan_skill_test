...
class AnswerOptionInline(admin.TabularInline):
    ...
    extra = 4
    min_num = 2
    fields = ('order_index', 'label', 'is_correct')
...
class QuestionInline(admin.StackedInline):
    ...
    # NOTE: do NOT add classes=('collapse',) here.
    # With `changeform_format = "horizontal_tabs"` (Jazzmin) the Questions tab
    # represents this inline. Marking it as `collapse` hides the section by
    # default, which makes the Questions tab appear empty when no questions
    # have been saved yet. We keep it expanded by default so the user can
    # immediately see the "Add another Question" button.
...
class TestAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Test Information', {
            'fields': ('title', 'description', 'category', 'is_actual_exam_demo'),
...
class QuestionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Question Content', {
            'fields': ('test', 'section', 'type', 'instruction', 'prompt', 'order_index'),
        }),
